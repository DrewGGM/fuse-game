...
Three changes came out of that. The file is tracked against Capacitor's
`.gitignore`, so a regeneration shows up as a diff. `read()` in the guard suite
throws a specific error when a file is absent, because **a hardened file that is
not in the repository is a finding, not a broken test.** And because measurement
showed `cap sync` restores the wildcard on *every* run, the repair is automated
rather than remembered:

```bash
npm run android:sync      # build, sync, then reapply hardening
npm run android:harden    # reapply on its own; idempotent, reports what changed
```

`scripts/harden-android.mjs` owns the two files Capacitor regenerates wholesale
and patches `allowBackup` in the manifest it only partly owns. Hand-editing a
generated file is a rule that lasts until someone forgets; a script in the sync
path is not.
